...
<!-- tensortonic:start -->
# Nikhesh Kumar Murali's TensorTonic Solutions

Verified machine learning implementations completed on [TensorTonic](https://www.tensortonic.com).

<p align="center">
  <img src="https://www.tensortonic.com/api/badge/nk10.svg" alt="TensorTonic Verified Solutions" width="100%" />
</p>
...
| Depth-Scaled Sandwich Norm | Implement Arcee Trinity sandwich normalization around sublayers with depth-dependent residual scaling. | https://www.tensortonic.com/research/arcee-trinity/at-sandwich-norm |
...
| Decoupled RoPE Key Assembly | Apply DeepSeek V3 decoupled RoPE to the positional key and query components outside the compressed content path. | https://www.tensortonic.com/research/deepseekv3/ds3-decoupled-rope |
| Dense Prefix and Sparse MoE Transition | Route early DeepSeek V3 Transformer layers through dense feed-forward blocks before switching to sparse MoE. | https://www.tensortonic.com/research/deepseekv3/ds3-dense-prefix |
| KV Latent Compression | Implement DeepSeek V3 KV compression by projecting hidden states into a shared low-rank attention latent. | https://www.tensortonic.com/research/deepseekv3/ds3-kv-compress |
| Head-Specific KV Reconstruction | Reconstruct DeepSeek V3 key and value representations by up-projecting the compressed KV latent for each head. | https://www.tensortonic.com/research/deepseekv3/ds3-kv-reconstruct |
| Auxiliary-Loss-Free Expert Bias Update | Implement DeepSeek V3 auxiliary-loss-free load balancing by updating expert routing biases from observed loads. | https://www.tensortonic.com/research/deepseekv3/ds3-load-balance |
| Multi-head Latent Attention | Build DeepSeek V3 multi-head latent attention with KV compression, decoupled RoPE, causal masking, and output projection. | https://www.tensortonic.com/research/deepseekv3/ds3-mla |
| Normalized Sigmoid Expert Router | Implement the DeepSeek V3 MoE router with expert scores, top-k selection, and normalized routing weights. | https://www.tensortonic.com/research/deepseekv3/ds3-moe-router |
| Multi-Token Prediction Module Input | Implement the DeepSeek V3 multi-token prediction head that produces logits for multiple future-token offsets. | https://www.tensortonic.com/research/deepseekv3/ds3-mtp |
| Always-Active Shared Expert | Implement the DeepSeek V3 shared expert path and combine its dense output with routed expert outputs. | https://www.tensortonic.com/research/deepseekv3/ds3-shared-expert |
...
| Gemma 3 Attention Sublayer | Implement a Gemma 3 attention block with QK normalization, rotary embeddings, and global or sliding-window masks. | https://www.tensortonic.com/research/gemma3/gemma3-attention-block |
| Full Gemma 3 Transformer Block | Assemble a Gemma 3 Transformer block with normalized attention, gated feed-forward computation, and residual paths. | https://www.tensortonic.com/research/gemma3/gemma3-full-block |
...
| Shared Expert with Routed Scaling | Implement the GLM-4.5 shared expert path and combine it with routed experts using the configured scaling factor. | https://www.tensortonic.com/research/glm45/glm-shared-expert |
...
| Causal Scaled Dot-Product Attention | Implement GPT-2 causal self-attention with an upper-triangular mask that blocks access to future tokens. | https://www.tensortonic.com/research/gpt2/gpt2-causal-attention |
| GPT-2 Pre-Norm Decoder Block | Assemble a pre-normalized GPT-2 decoder block with causal attention, feed-forward layers, and residual connections. | https://www.tensortonic.com/research/gpt2/gpt2-decoder-block |
| Token and Learned Position Embeddings | Build GPT-2 input representations by summing learned token embeddings with learned positional embeddings. | https://www.tensortonic.com/research/gpt2/gpt2-embedding |
| GPT-2 Position-Wise Feedforward Network | Implement the GPT-2 position-wise feed-forward network with expansion, GELU activation, and output projection. | https://www.tensortonic.com/research/gpt2/gpt2-ffn |
| GPT-2 Full Forward Pass | Build the GPT-2 forward pass from token and position embeddings through decoder blocks to vocabulary logits. | https://www.tensortonic.com/research/gpt2/gpt2-forward |
| GPT-2 GELU Activation | Implement GPT-2's tanh-based GELU approximation for smooth nonlinear activation of feed-forward features. | https://www.tensortonic.com/research/gpt2/gpt2-gelu |
| Greedy Autoregressive Decoding | Implement GPT-2 greedy decoding by repeatedly selecting the highest-logit next token from the model output. | https://www.tensortonic.com/research/gpt2/gpt2-greedy-decode |
| Residual Projection Initialization Scaling | Compute GPT-2 residual projection initialization scaling based on model width and decoder depth. | https://www.tensortonic.com/research/gpt2/gpt2-init-scaling |
| GPT-2 Layer Normalization | Implement GPT-2 layer normalization with per-token statistics and learned scale and bias parameters. | https://www.tensortonic.com/research/gpt2/gpt2-layernorm |
...
| Implement Sparse MoE Top-k Expert Routing | Select the top k highest-scoring experts per token and compute routing weights as a softmax over only those selected logits. | https://www.tensortonic.com/study-plans/cracking-inference/inference-moe-top-k-routing |
...
| Implement Per-Channel Weight Quantization | Quantize weights independently along a selected channel axis, then dequantize with channel-specific scales. | https://www.tensortonic.com/study-plans/cracking-inference/inference-per-channel-quantization |
...
| Implement Temperature, Top-k, and Top-p Sampling | Sample tokens from batched logits using temperature scaling, top-k filtering, top-p filtering, and deterministic randomness. | https://www.tensortonic.com/study-plans/cracking-inference/inference-token-sampling |
...
| Padding and Truncation | Pad or truncate integer token sequences to a fixed length with configurable padding value and alignment direction. | https://www.tensortonic.com/study-plans/cracking-nlp/nlp-padding-truncation |
| Regex Patterns for NLP | Implement Regex Patterns for NLP, and return the list of matched strings in order of appearance. | https://www.tensortonic.com/study-plans/cracking-nlp/nlp-regex-patterns |
...
| GRPO Group-Relative Advantage | Normalize rewards within each GRPO group using its mean and standard deviation, including zero-variance handling. | https://www.tensortonic.com/study-plans/cracking-rl/rl-grpo-group-relative-advantage |
...
| RLOO (REINFORCE Leave-One-Out) | RLOO (REINFORCE Leave-One-Out) is a policy gradient variance-reduction trick used in modern RLHF. | https://www.tensortonic.com/study-plans/cracking-rl/rl-rloo-reinforce-leave-one-out |
...
| Batch vs Stochastic vs Mini-Batch GD | Compare how different batch sizes affect gradient descent on linear regression, and return a list of loss curves (one per batch size). | https://www.tensortonic.com/study-plans/math-optimization/optim-batch-vs-sgd |
| Classical Momentum | Implement vanilla gradient descent and momentum-based gradient descent to minimize the MSE loss of a linear model, and return both loss curves. | https://www.tensortonic.com/study-plans/math-optimization/optim-classical-momentum |
